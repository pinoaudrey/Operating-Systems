...
int main(int argc, char *argv[])
{
  ...
  if (pid_1 == 0) {
    /* First Child */
  // set char array to correct size
  char cmdbuf[BSIZE];
  // fill cmdbuf with zeros
  bzero(cmdbuf, BSIZE);
  // fill array with cmd line code that we want FIND_EXEC to run (string arg & directory)
  sprintf(cmdbuf, "%s %s -name \'*\'.[ch]", FIND_EXEC, argv[1]);
    ...
    // run shell cmd & check for errors
